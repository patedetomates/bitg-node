...
const storeTransaction = (request, response) => {
    let { blockNumber, hash, sender, recipient, amount, gasFees, date } = request
    date = new Date(parseInt(date)).toISOString()

    pool.query('INSERT INTO transactions ("blockNumber", "hash", "sender", "recipient", "amount", "gasFees", "date") VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [blockNumber, hash, sender, recipient, amount, gasFees, date], (error, results) => {
        if (error) {
            throw error
        }
